...
#include "../src/apps/atApp_Web_Server.h"
...
// || Creation           ||     Task function         ||     PC name                    || heap size  || Parameter || Priority  || Task handle          || CoreID   ||
  ...
  xTaskCreatePinnedToCore(  atApp_Web_Server_Task_Func, "atApp_Web_Server_Application"  ,  100000      ,     NULL    ,   2       , &Task_atApp_Web_Server,    1     );
  ...
  // xTaskCreatePinnedToCore(  atApp_exHMI_Task_Func      , "atApp_exHMI_Application"       ,  10000        ,     NULL    ,   1       , &Task_atApp_ABC       ,    0     );
//------------------------------------------------------------------------------------------------------------------------------------------------------------------//
// Some tasks will be suspended after initing 
  // vTaskSuspend(Task_atApp_MB_TCP_SL);
  vTaskSuspend(Task_atApp_Web_Server);
}
